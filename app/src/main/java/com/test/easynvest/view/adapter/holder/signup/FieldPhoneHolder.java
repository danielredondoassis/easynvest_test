...
public class FieldPhoneHolder extends BaseHolder implements BaseHolder.HolderListeners {
    ...
    public FieldPhoneHolder(View itemView) {
        super(itemView);

        mEditInput = itemView.findViewById(R.id.editPhonetInput);
        mEditText = itemView.findViewById(R.id.editPhoneText);
    ...
    public void setup(Context context, SignUpContracts.Interactor interactor, CellModel cellModel) {
        ...
        if(mTypeField != null){
            if(mTypeField == TypeField.PHONE){
                mEditText.setInputType(InputType.TYPE_TEXT_FLAG_NO_SUGGESTIONS | InputType.TYPE_CLASS_PHONE);
                mEditText.isPhone(true);
                mEditText.setMask("(99) 99999-9999");
            }
        }
